...
function getType(score: number): FitnessType {
  if (score <= 6)
    return {
      name: "방전된 직장인형",
      stage: "1단계 · 습관화",
      desc: "지금은 체력 배터리가 거의 바닥난 상태예요. 운동량을 늘리기보다, 일단 '움직이는 사람'이 되는 게 먼저입니다. 아주 작게 시작해서 끊기지 않게 가는 게 핵심이에요.",
      productId: "ebook",
      productName: "전자책",
    };
  if (score <= 13)
    return {
      name: "책상 고정형",
      stage: "3단계 · 기초 연습",
      desc: "기본 체력은 있는데 오래 앉아 몸이 굳었네요. 제대로 된 기초 동작부터 익히면 생각보다 빨리 바뀝니다. 자세와 원리를 잡는 단계예요.",
      productId: "ebook",
      productName: "전자책",
    };
  if (score <= 19)
    return {
      name: "작심삼일 성장형",
      stage: "4단계 · 체력 성장",
      desc: "체력 토대는 충분합니다. 이제 강도를 꾸준히 올려 '성장'할 차례예요. 루틴을 체계화하면 확실히 한 단계 올라섭니다.",
      productId: "program",
      productName: "운동 프로그램",
    };
  return {
