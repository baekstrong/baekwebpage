...
function getType(score: number): FitnessType {
  if (score <= 6)
    return {
      name: "방전된 직장인형",
      stage: "1단계 · 습관화",
      desc: "솔직히 지금 체력 배터리는 거의 바닥입니다. 여기서 운동량부터 늘리면 십중팔구 3일 만에 나가떨어져요. 그래서 1단계 목표는 '운동을 잘하는 사람'이 아니라 일단 '몸을 움직이는 사람'이 되는 겁니다. 하루 5분, 계단 한 층이라도 좋아요. 강도보다 '끊기지 않는 것' 하나에만 집중하면 2주 안에 몸이 먼저 달라집니다.",
      productId: "ebook",
      productName: "전자책",
    };
  if (score <= 13)
    return {
      name: "책상 고정형",
      stage: "3단계 · 기초 연습",
      desc: "기본 체력은 있는데, 오래 앉아 일하느라 몸이 굳고 약해진 곳이 생긴 상태예요. 의외로 이 단계가 가장 빨리 바뀝니다. 없는 체력을 만드는 게 아니라 굳은 몸을 깨우고 잘못된 자세를 바로잡는 거라서요. 힙힌지·복압 같은 기본 동작의 '기준'부터 잡으면, 같은 운동을 해도 효과가 확 달라집니다.",
      productId: "ebook",
      productName: "전자책",
    };
  if (score <= 19)
    return {
      name: "작심삼일 성장형",
      stage: "4단계 · 체력 성장",
      desc: "체력 토대는 이미 충분합니다. 문제는 '꾸준함'이 아니라 '체계'예요. 그때그때 내키는 대로 운동하니 늘 제자리인 겁니다. 강도를 단계적으로 올리는 루틴을 잡고 주차별로 뭘 얼마나 할지 정해두면, 작심삼일이 들어올 틈이 없어요. 여기서부터는 눈에 보이게 성장합니다.",
      productId: "program",
      productName: "운동 프로그램",
    };
  return {
    name: "강철 직장인형",
    stage: "5단계 · 체력 끝판왕",
    desc: "이미 상위권 체력입니다. 웬만한 운동으론 더 안 늘어요. 여기서부터는 '많이'가 아니라 '정확하게'의 영역입니다. 약점 부위를 골라 보완하고, 케틀벨 같은 도구로 심폐와 근력을 동시에 끌어올릴 때예요. 혼자 하면 한계가 오니, 체계적인 정규 과정으로 마지막 한 끗을 채우는 걸 추천합니다.",
    productId: "school",
        ...
        <div className="wrap quiz-wrap">
          {hasPain ? (
            <div className="quiz-result">
              ...
              <p className="qr-lead">
                통증이 있는데 무리해서 운동하면 오히려 더 망가집니다.
                지금은 강도를 올리기보다, 아픈 곳을 안전하게 다루는 게
                우선이에요.
              </p>
              <p>
